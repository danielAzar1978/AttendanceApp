...
The mobile application supports offline attendance recording.

Flow:

1. The employee performs Check-In or Check-Out.
2. The application captures GPS coordinates and device time.
3. The application attempts to send the record to the API.
4. If the request fails because the server or network is unavailable, the record is stored locally in AsyncStorage.
5. Pending records are synchronized automatically when connectivity is restored or when the user returns to the main screen.
6. Successfully synchronized records are removed from local storage.
...
Each attendance record receives a unique ClientRecordId generated by the mobile application.

The API checks whether the ClientRecordId already exists before saving.

This prevents the same offline record from being saved more than once.

### Attendance Sequence Validation

The API prevents:

- Two Check-In records in sequence on the same day
- Two Check-Out records in sequence on the same day

The validation is performed only against records from the attendance date.

A record from a previous day must not block a valid attendance action on a new day.
...
- Offline records use ClientRecordId for idempotency.
- AttendanceTime is generated on the mobile device and preserved during delayed synchronization.
- Pending records rejected by business validation are removed from the local queue.
- Temporary server failures keep records in the local queue for retry.
- Network connectivity changes are monitored using @react-native-community/netinfo.
